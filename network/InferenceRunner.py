...
class InferenceRunner(NetRunner):
    ...
    def _run_tensorflow_pipeline(self):
        ...
        with tf.Session(graph=graph, config=config) as sess:
            ...
            count = 0
            for i in tqdm(len(self.inference_X), total=len(self.inference_X), unit=' steps', desc='Inference'):
                start_time = time.time()
                ff = list(i)
                if self.task_type == 'classification':
                    pred_result_ = sess.run(pred, feed_dict={
                        X: [np.reshape(f[0], [self.img_size[0], self.img_size[1], self.img_size[2]]) for f
                            in ff], mode: False})
                    ...
                    if self.gradcam_record:
                        gradcam_params = sess.run(gradcam_op, feed_dict={
                            X: [np.reshape(f[0], [self.img_size[0], self.img_size[1], self.img_size[2]]) for f
                                in ff], mode: False, y_true: pred_result_})
                        pred_result.extend(pred_result_)
                        gradcam_results_ = []
                        # Run Grad-CAM
                    ...
                    for fr in ff:
                        ...
                        ax2.imshow(np.reshape(m_r, [self.img_size[0], self.img_size[1]]))
                        ax2.set(xlabel='generated segmentation')
                        ax1.imshow(np.reshape(fr[0], [self.img_size[0], self.img_size[1]]))
                        ax1.set(xlabel='original image')
            ...
            if self.task_type == 'classification':
                ...
                if self.loss_type == 'mse':
                    m = 0
                    y_pred = np.array(pred_result)
                    y_true = np.array(h5_file['lbl_test'][:])
                    ...
                    y_pred = np.argmax(np.array(pred_result).squeeze(), axis=1)
